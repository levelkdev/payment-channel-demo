...
class App extends Component {
  ...
  async loginMetamask() {
    await this.props.requestLoginNonce()
    const sig = await signMessage(this.props.loginNonce)
    const account = getConnectedAccount()
    await this.props.loginMetamask(sig, account)
    if (this.props.loggedIn) {
      this.props.history.push('/deploy')
    }
  }
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    ...
    loginMetamask: (sig, acct) => dispatch(loginMetamask(sig, acct)),
    requestLoginNonce: () => dispatch(requestLoginNonce()),
  }
}
